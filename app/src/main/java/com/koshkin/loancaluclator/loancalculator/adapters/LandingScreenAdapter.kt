 ...
 */
class LandingScreenAdapter(var loans: Loans, var paymentsList: PaymentsList) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup?, viewType: Int): RecyclerView.ViewHolder {
        when (viewType) {
            0 -> return ChartHolder(LayoutInflater.from(parent!!.context).inflate(com.koshkin.loancaluclator.loancalculator.R.layout.adapter_landing_view_chart, parent, false))
            2 -> return LoadingHolder(LayoutInflater.from(parent!!.context).inflate(R.layout.adapter_loading_layout, parent, false))
    ...
    override fun getItemViewType(position: Int): Int {
        when (position) {
            ...
            1 -> if (loans.status == LoadingStatus.LOADING) return VIEW_TYPE_LOADING else return VIEW_TYPE_LOAN
